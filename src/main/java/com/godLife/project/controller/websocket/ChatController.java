...
public class ChatController {
  ...
  @SendTo("/sub/waitList")
  public WaitListMessageDTO broadcastWaitList() {
    return qnaService.getlistAllWaitQna(MessageStatus.RELOAD.getStatus());
  }
  ...
  public void sendMatchedQnaList(@Header("Authorization") String authHeader,
    ...
    MatchedListMessageDTO request = qnaService.getlistAllMatchedQna(adminIdx, MessageStatus.RELOAD.getStatus());
  ...
  public void takeWaitQna(@Header("Authorization") String authHeader,
    ...
    if (qnaIdx == 0) {
      throw new WebSocketBusinessException("문의를 선택해 주세요. 해당 문의가 삭제 됐거나, null 혹은 0 으로 요청했습니다.", 4004);
    }
    if (adminInfo.getUserIdx() == 0) {
      throw new WebSocketBusinessException("유효하지 않은 jwt 토큰 이거나, 존재하지 않은 관리자 입니다.", 4003);
    }
  ...
  public void getMatchedQnaDetail(@Header("Authorization") String authHeader,
    ...
    try {
      if (qnaIdx <= 0) {
        throw new WebSocketBusinessException("문의 인덱스를 선택해주세요. 문의 인덱스는 0 보다 커야 합니다.", 4000);
      }
      if (adminIdx == 0) {
      ...
      if (e.getStatus() == HttpStatus.FORBIDDEN) {
        throw new WebSocketBusinessException(e.getMessage(), 4003);
      }
